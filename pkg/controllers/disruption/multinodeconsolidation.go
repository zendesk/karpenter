...
func (m *MultiNodeConsolidation) ComputeCommands(ctx context.Context, disruptionBudgetMapping map[string]int, candidates ...*Candidate) ([]Command, error) {
	...
	// create 1 command per node-type, making the checks much simpler since no candidate should get rejected
	...
	groupedCandidates := lo.GroupBy(disruptableCandidates, func(candidate *Candidate) string {
		return candidate.Labels()["node-type"]
	})
	for nodeType, cs := range groupedCandidates {
		...
		if err != nil {
			log.FromContext(ctx).V(1).Error(err, "ignoring failed multinodeconsolidation option", "node-type", nodeType)
			continue
		}
